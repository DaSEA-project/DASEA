...
doctl compute ssh DASEA-tool-miner-1 --ssh-command "source ~/.profile && git clone git@github.com:dependulum/DASEA.git && cd DASEA && poetry install && poetry run dasea mine alire && scp -i ~/.ssh/miner1 -r ./data/out root@157.245.70.200:/root/DASEA/data/out"
