...
doctl compute ssh DASEA-tool-miner-1 --ssh-command "source ~/.profile && git clone https://github.com/dependulum/DASEA.git && cd DASEA && poetry install && poetry run dasea mine alire && scp -i ~/.ssh/id_rsa -r ./data/out root@157.245.70.200:/root/DASEA/data/out"
