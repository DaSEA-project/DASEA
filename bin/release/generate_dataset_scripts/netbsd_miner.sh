...
doctl compute droplet create --image 102629764 --size s-4vcpu-8gb --region ams3 --wait --ssh-keys 33315393 DASEA-tool-netBSD-miner

## Sleep while the droplet boots up
sleep 45

## Get the droplet's IP address
IP_ADDRESS=$(doctl compute droplet get --format "PublicIPv4" --no-header DASEA-tool-netBSD-miner)
...
doctl compute ssh DASEA-tool-netBSD-miner --ssh-key-path ~/.ssh/dasea --ssh-command "
...
doctl compute droplet delete DASEA-tool-netBSD-miner --force
