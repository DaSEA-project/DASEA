...
doctl compute droplet create --image 102629764 --size s-4vcpu-8gb --region ams3 --wait --ssh-keys 33315393 DASEA-tool-rubygems-miner

## Sleep while the droplet boots up
sleep 45

## Get the droplet's IP address
IP_ADDRESS=$(doctl compute droplet get --format "PublicIPv4" --no-header DASEA-tool-rubygems-miner)
...
## SSH into the droplet, Execute RubyGems miner, SCP back data to STEVE
echo "SSH into the droplet..."
doctl compute ssh DASEA-tool-rubygems-miner --ssh-key-path ~/.ssh/dasea --ssh-command "
...
doctl compute droplet delete DASEA-tool-rubygems-miner --force
